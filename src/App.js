...
function App() {
  ...
  const [currencyIn, setCurrencyIn] = React.useState(['currency', 0])
  const [currencyOut, setCurrencyOut] = React.useState(['currency', 0])
  ...
  function selectAmountIn(amount) {
    setAmountIn(amount)
    const exchangeRate = currencyOut[1] / currencyIn[1]
    setAmountOut(amount * exchangeRate)
  }

  function selectAmountOut(amount) {
    setAmountOut(amount)
    const exchangeRate = currencyIn[1] / currencyOut[1]
    setAmountIn(amount * exchangeRate)
  }
